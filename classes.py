...
class AddressBook(UserDict, AbstractShow):
...
class Record:
    ...
    def __str__(self):
        return f"Name: {self.name.value} Phone: {self.phones} Address: {self.address.value if self.address else None} Email: {self.email.value if self.email else None} Birthday: {self.birthday.value if self.birthday else None}"
...
class NoteBook(UserDict, AbstractShow):
